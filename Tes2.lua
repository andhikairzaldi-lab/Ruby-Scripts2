    ...
    if Toggle then
        task.spawn(function()
            while Toggle do
                ...
                local trainingRemote = network:FindFirstChild("rev_TrainingEvent") or network:FindFirstChild("rev_ClickEvent") -- Sesuaikan nama remotenya jika salah
if trainingRemote then
    -- Mengirim argumen "x2" atau "Multiplier" ke server (sesuai data game)
    trainingRemote:FireServer("x2")       
                if hum then
